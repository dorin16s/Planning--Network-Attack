...
namespace PDDL
{
    public class PartiallySpecifiedState
    {
        ...
        public HashSet<Predicate> m_lOfflineObservedPredicates;
            ...
            if (GeneratingAction.Effects != null)
            {
                ...
                foreach (Predicate p in aRevised.GetMandatoryEffects())
                {
                    if (!m_lObserved.Contains(p))
                    {
                        ...
                        if (!Problem.Domain.IsSimple || (!SDRPlanner.OptimizeMemoryConsumption && !SDRPlanner.ComputeCompletePlanTree))
                            AddObserved(p);
                    }
                }
                ...
                if (!SDRPlanner.ComputeCompletePlanTree || !Problem.Domain.IsSimple)
        ...
        {
            m_bsInitialBelief.MaintainProblematicTag = false;
        ...
        public void WriteSimpleProjectionDomainAndProblem(string sDomainFile, string sProblemFile, CompoundFormula cfGoal, out MemoryStream msModels)
        {
            MemoryStream msProblem = Problem.WriteSimpleProblem(sProblemFile, this, cfGoal);
        ...
        public void UpdateClosedStates(Dictionary<string,List<PartiallySpecifiedState>> dClosedStates ,Domain d, bool bAlreadyClosed)
        {
            ...
            while (pssIter.Predecessor != null)
            {
                ...
                else
                {
                    ...
                    pssIter.Predecessor.m_lOfflineObservedPredicates = new HashSet<Predicate>(pssIter.m_lOfflineObservedPredicates);
                    pssIter.Predecessor.m_lOfflineObservedPredicates.UnionWith(pssIter.Predecessor.m_pssFirstChild.m_lOfflineObservedPredicates);
                    pssIter.Predecessor.m_lOfflineObservedPredicates.Add(((PredicateFormula)pssIter.GeneratingObservation).Predicate.Canonical());
        ...
        public bool IsClosedStateNonSimple(Dictionary<string, List<PartiallySpecifiedState>> dClosedStates)
        {
            if (IsGoalState())
                return true;
            foreach (KeyValuePair<string, List<PartiallySpecifiedState>> p in dClosedStates)
            {
                foreach (PartiallySpecifiedState pssClosed in p.Value)
                {
                    if (pssClosed.Observed.Equals(Observed) && pssClosed.Hidden.Equals(Hidden))
                    {
                        bool bCheck = pssClosed.Plan.ValidatePlan(this);
                        if (bCheck)
                        {
        ...
        {
            if (!Problem.Domain.IsSimple)
                return IsClosedStateNonSimple(dClosedStates);
        ...
        public Predicate FindRequiredObservable(Dictionary<string, List<PartiallySpecifiedState>> dClosedStates)
        {
            if (!Problem.Domain.IsSimple)
                return null;
            ...
            int cMaxObserved = 0;
            Predicate pMaxObserved = null;
            Dictionary<Predicate,int> dObservedInClosed = new Dictionary<Predicate,int>();
            //foreach (List<PartiallySpecifiedState> dc in dClosedStates.Values)
            //{
            List<PartiallySpecifiedState> dc = dClosedStates[ToString()];
            if (dc == null)
                return null;
                foreach (PartiallySpecifiedState pssClosed in dc)
                {
                    foreach (GroundedPredicate gp in pssClosed.m_lOfflineObservedPredicates)
                    {
                        if (Hidden.Contains(gp))
                        {
                            if (!dObservedInClosed.ContainsKey(gp))
                                dObservedInClosed[gp] = 0;
                            dObservedInClosed[gp]++;
                            if (dObservedInClosed[gp] > cMaxObserved)
                            {
                                cMaxObserved = dObservedInClosed[gp];
                                pMaxObserved = gp;
                            }
                        }
                    }
            ...
            //}
            //if (pMaxObserved != null)
                //Console.Write(pMaxObserved + " ");
            return pMaxObserved;
        }
