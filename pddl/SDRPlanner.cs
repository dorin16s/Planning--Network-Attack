...
namespace PDDL
{
    class SDRPlanner
    {
        ...
        public static TimeSpan PlannerTimeout = new TimeSpan(0, 0, 30);
            ...
            if (m_sFFOutput.Contains("found legal plan as follows"))
            {
                ...
                for (int i = 2; i < asPlan.Length; i++)
                {
                    if (!asPlan[i].Contains(":"))
                        break;
                    string sAction = asPlan[i].Substring(asPlan[i].IndexOf(':') + 2).Trim().ToLower();
                    if(sAction != "reach-goal")
                        lPlan.Add(sAction);
                }
            }
        ...
        private Dictionary<Predicate, HashSet<Action>> m_dObservationActions = new Dictionary<Predicate, HashSet<Action>>();

        private List<string> Plan(string sPath, PartiallySpecifiedState pssCurrent, Predicate pObservationGoal)
        {
            ...
            foreach (Action a in m_dObservationActions[pObservationGoal])
                cfGoal.AddOperand(a.Preconditions);

            MemoryStream msModels = null;
            pssCurrent.WriteSimpleProjectionDomainAndProblem(sPath + "Kd.pddl", sPath + "Kp.pddl", cfGoal, out msModels);
            ...
            lPlan.Add(m_dObservationActions[pObservationGoal].First().Name.Replace("_"," "));
        ...
        public void OfflinePlanning(string sPath, Domain domain, Problem problem)
        {
            ...
            while (!bDone)
            {
                ...
                while (lPlan == null || lPlan.Count == 0)
                {
                    ...
                    if (!pssCurrent.IsClosedState(dClosedStates))
                    {
                        ...
                        /*
                        Predicate pObserve = pssCurrent.FindRequiredObservable(dClosedStates);
                        //pObserve = null;
                        if (pObserve != null)
                            lPlan = Plan(sPath, pssCurrent, pObserve);
                        else
                            lPlan = null;
                        if(lPlan == null)
                         * */
                    ...
                    foreach (string sAction in lPlan)
                    {
                        if (!IsReasoningAction(sAction.ToLower()))
                        {
                            ...
                            if (a.Observe != null)
                            {
                                ...
                                if(!m_dObservationActions.ContainsKey(pObserve))
                                    m_dObservationActions[pObserve] = new HashSet<Action>();
                                m_dObservationActions[pObserve].Add(a);
                            }
            ...
            */
            ...
            Console.WriteLine("Done - time: " + tsTotal.TotalSeconds);
        }
